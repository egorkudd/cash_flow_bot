...
public class StatisticEndDateChoiceStep extends ChoiceStep {
    ...
    protected int finishStep(TgChat tgChat, AbsSender sender, Update update) throws AbstractBotException {
        if (InlineCalendarCommandUtil.isInlineCalendarClicked(update)){
            ...
            statisticService.setEndDate(dateStr, user.getId());
            MessageUtil.sendMessage(UpdateUtil.getChatId(update), "Конец: ".concat(dateStr), sender);
            sendStatistic(tgChat, user.getId(), sender);
            return 0;
        }
    ...
    private void sendStatistic(TgChat tgChat, Long userId, AbsSender sender) {

        Statistic statistic = statisticService.setCreatedAt(userId);
        StatisticInfo info = statisticService.getTransactionStatisticByCustomTimeInterval(
                userId,
                statistic.getStartDate().toInstant(),
                statistic.getEndDate().toInstant()
        );

        String messageText = statisticService.collectStatisticMessage(info, "%s - %s".formatted(
                DateUtil.convertDate(statistic.getStartDate()), DateUtil.convertDate(statistic.getEndDate())
        ));
